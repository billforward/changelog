...
Trials are now defined on a Product Rate Plan â€” with default values being specified on the associated Product. When a subscription is of type â€˜Trialâ€™, any charges that are created will have the â€˜trialâ€™ flag set to â€˜trueâ€™.  When an Invoice is created for these Charges they will receive a 100% discount.  This will result in an Invoice with zero total cost, but with all other values the same (as though the Subscription were not currently in a trial).
